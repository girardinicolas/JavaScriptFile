...
export function mostraSoloContainer(filter) {
    ...
    if (filter === 'all' || !filter) {
        ...
        Object.keys(containers).forEach(key => {
            ...
            if (c)
                c.style.display = (key === filter) ? 'grid' : 'none';
        });
    }
}
export function renderList() {
    ...
    if (!toWatchContainer || !ongoingContainer || !seenContainer)
        return;
    ...
    list.sort((a, b) => currentSortOrder === 'asc' ? a.addedDate - b.addedDate : b.addedDate - a.addedDate);
    list.forEach((anime, index) => {
        ...
        ['toWatch', 'ongoing', 'seen'].forEach(status => {
            ...
            cb.addEventListener('change', e => updateStatus(index, statusKey, e.target.checked));
        ...
        if (anime.seen)
            seenContainer.appendChild(card);
        else if (anime.ongoing)
            ongoingContainer.appendChild(card);
        else
            toWatchContainer.appendChild(card);
    });
}
export function updateCounters() {
    ...
    if (toWatchCountElem)
        toWatchCountElem.textContent = toWatchCount.toString();
    if (ongoingCountElem)
        ongoingCountElem.textContent = ongoingCount.toString();
    if (seenCountElem)
        seenCountElem.textContent = seenCount.toString();
}
